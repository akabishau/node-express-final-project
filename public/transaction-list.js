        ...
        if (response.status === 200) {
            if (data.count === 0) {
                ...
                for (let i = 0; i < data.transactions.length; i++) {
                    let editButton = `<td><button type="button" class="editButton" data-id=${data.transactions[i]._id}>edit</button></td>`;
                    let deleteButton = `<td><button type="button" class="deleteButton" data-id=${data.transactions[i]._id}>delete</button></td>`;

                    let rowHTML = ` <td>${data.transactions[i].transType}</td>
